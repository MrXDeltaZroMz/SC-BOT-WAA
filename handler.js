...
case 'tovirgam':
  case 'jadivirgam':
    case 'tobug':
      reply(mess.wait)
         if(!isQuotedSticker) return data.reply('reply stickernya bukan gambarnya goblok!')
         if(message.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated) {
         	const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							client.sendMessage(from, buffer, image, {quoted: mek, thumbnail:virgam, caption: 'Nih Fotonya\n\nâš ï¸*Perhatian.!!*âš ï¸\nFoto ini Mengandung Virgam/Bug SW!!'})
							fs.unlinkSync(ran)
						})
					}
					break
